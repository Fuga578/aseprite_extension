...
#### ユニットテスト(Asepriteランタイム外 / busted)

**対象**: 純粋ロジック — `CommandBuilder`、`HistoryStore`、`lib/strings`、`lib/platform`

**方法**: `tests/support/aseprite-mock.lua` で Aseprite API をモックし、busted で実行
...
| Lua | 5.4 | ユニットテスト実行 | busted利用時に必要 |
| busted | 2.x | ユニットテスト | 任意(推奨) |
| luacheck | 0.26+ | 静的解析 | 任意(推奨) |
...
# 2. (任意) テスト・Lintツールの導入
#    LuaRocks 経由で busted / luacheck をインストール
